...
The objects from which transmittance can differ in additional properties that affect possible calculations. Attribute `filter.properties` is used to store these in an object with fields `Rfr.constant`, `thickness` and `homogeneous`. When these metadata are available, in many cases we can use function `convertTfrType()` to convert internal transmittance into total transmittance and _vice versa_ and function `convertThickness()` to compute the spectral transmittance of a filter of the same material but different thickness.
